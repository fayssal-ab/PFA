...
export default function MesReclamationsPage() {
  ...
  const [reponses, setReponses] = useState<ReponseReclamation[]>([]);
  ...
  const loadReclamationDetail = async (reclamationId: number) => {
    ...
    try {
      const [recRes, repRes] = await Promise.all([
        ...
        api.get<ApiResponse<ReponseReclamation>>(`/reponse-reclamations/get-reponse/${reclamationId}`, {
          params: { page: 0, size: 50 }
        })
      ]);
      setSelectedReclamation(recRes.data);
      setReponses(repRes.data?.content || []);
    } catch (error) {
  ...
  const priorityColor = (priority?: string) => {
    if (priority === "haute" || priority === "urgent") return "bg-red-100 text-red-700";
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gray-50 dark:bg-slate-800/50 rounded-xl p-5">
            <div className="flex flex-wrap gap-3 mb-4">
              <span className={`text-xs px-2 py-1 rounded-full ${statusColor(selectedReclamation.status?.status)}`}>
                {selectedReclamation.status?.status || "—"}
              </span>
              <span className={`text-xs px-2 py-1 rounded-full ${priorityColor(selectedReclamation.priority?.priority)}`}>
            ...
            <p className="text-xs text-gray-400 mt-4">
              Déposée le {selectedReclamation.dateDepot ? new Date(selectedReclamation.dateDepot).toLocaleDateString("fr-FR") : "—"}
            </p>
          </div>

          {/* Responses Section */}
          <div>
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Échanges</h3>
            <div className="space-y-4">
              {reponses.map((rep) => (
                <div key={rep.id} className="bg-white dark:bg-slate-800 rounded-xl p-4 border border-gray-100 dark:border-gray-700">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-gray-600 dark:text-gray-300">{rep.reponse}</p>
                    </div>
                  </div>
                </div>
              ))}
    ...
    >
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full ${priorityColor(rec.priority?.priority)}`}>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
          <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-lg p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-xl font-bold text-gray-900 dark:text-white">Nouvelle réclamation</h3>
