...
export default function AgentsPage() {
  ...
  const fetch = async () => {
    try { 
      const r = await api.get<ApiResponse<Agent>>("/agents/get-agents", { params: { page: 0, size: 100 } }); 
  ...
  return (
    <DashboardLayout>
      <div className="mb-6">
        ...
        <h1 className="text-[26px] font-bold text-[#1a1a2e] tracking-tight">Agents</h1>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        {[
          { label: "Total", value: agents.length, icon: Users, bg: "bg-indigo-50", iconBg: "bg-indigo-100", iconColor: "text-indigo-600" },
          { label: "Disponibles", value: dispo, icon: UserCheck, bg: "bg-emerald-50", iconBg: "bg-emerald-100", iconColor: "text-emerald-600" },
          { label: "Occupés", value: agents.length - dispo, icon: UserX, bg: "bg-amber-50", iconBg: "bg-amber-100", iconColor: "text-amber-600" },
        ].map(c => (
          <div key={c.label} className={`${c.bg} rounded-2xl p-5 ring-1 ring-gray-100`}>
            <div className="flex items-start justify-between">
              <div>
                <p className="text-[12px] text-gray-500 font-semibold uppercase tracking-wider">{c.label}</p>
                <p className="text-[28px] font-extrabold text-[#1a1a2e] mt-1 leading-none">{c.value}</p>
              </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={4} className="text-center py-16 text-gray-400 text-[14px]">Aucun agent</td></tr>
              ) : (
                filtered.map(a => (
                  <tr key={a.id} className="border-t border-gray-50/80 hover:bg-indigo-50/20 transition-colors">
                    <td className="px-6 py-3.5 text-[14px] text-[#1a1a2e] font-semibold">{a.user?.nom || "—"}</td>
                    <td className="px-6 py-3.5 text-[14px] text-gray-600">{a.user?.prenom || "—"}</td>
                    <td className="px-6 py-3.5 text-[13px] text-gray-500">{a.user?.email || "—"}</td>
